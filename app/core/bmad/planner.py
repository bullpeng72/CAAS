...
class SprintPlanner(LoggerMixin):
    ...
    def _group_into_sprints(
        ...
        for i, task in enumerate(sorted_tasks):
            ...
            if len(current_tasks) >= self.max_tasks_per_sprint or i == len(sorted_tasks) - 1:
                # 관련 에이전트 수집
                agent_ids = list(set(t.assigned_agent_id for t in current_tasks if t.assigned_agent_id))
    ...
    def optimize_plan(self, plan: SprintPlan) -> SprintPlan:
        ...
        for sprint in plan.sprints:
            ...
            optimized_sprint = sprint.model_copy(update={
        ...
        return plan.model_copy(update={
            ...
            "estimated_total_iterations": sum(s.estimated_iterations for s in optimized_sprints),
        })

...
| 카테고리 | 명령어 | 검사 항목 |
|---------|--------|----------|
| **보안 스캔** | `caas qa security-scan` | SQL Injection, XSS, Hardcoded Secrets, Command Injection |
| **코드 품질** | `caas qa code-quality` | Docstring, Type Hints, Complexity, PEP 8 |
| **성능 테스트** | `caas qa performance` | 응답 시간, 메모리 사용량, 동시성 |
| **통합 검증** | `caas qa validate-all` | 모든 검사 통합 실행 |
    ...
    steps:
      ...
      - name: Code Quality
        run: |
          ...
          # Fail if score < 8.5
          SCORE=$(caas qa code-quality --project . | grep "Overall Score" | awk '{print $3}' | cut -d'/' -f1)
          if (( $(echo "$SCORE < 8.5" | bc -l) )); then
            echo "❌ Code quality too low: $SCORE"
            exit 1
          fi
    ...
    stages {
        stage('QA Validation') {
            steps {
                ...
                // Security
                sh 'caas qa security-scan --project . --owasp'

                // Code Quality
                sh 'caas qa code-quality --project .'

                // Performance
                sh 'caas qa performance --project . --scenarios load_test.yml'

                // Generate Report
                sh 'caas qa validate-all --project . --output qa_report.html'
...
1. **CI/CD에 QA 통합**
   ```yaml
   - name: QA Gate
     run: caas qa validate-all --fail-on-critical
   ```

2. **정기적인 보안 스캔**
   ```bash
   # 매일 자정 실행 (crontab)
   0 0 * * * caas qa security-scan --project /app --email report@company.com
   ```
...
1. **Critical 이슈 무시**
   ```bash
   # 나쁜 예
   caas qa security-scan || true  # 무시하지 말 것
   ```

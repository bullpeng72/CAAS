...
```bash
# Streamlit UI 포함
caas generate "챗봇" --ui streamlit

# Gradio UI 포함
caas generate "이미지 분류기" --ui gradio

# UI 없음 (CLI만)
caas generate "데이터 처리" --no-ui
```
...
```bash
# 엄격한 품질 검증 (권장)
caas generate "요구사항" --strict-quality

# 빠른 생성 (품질 검증 최소화)
caas generate "요구사항" --fast

# 테스트 커버리지 목표 설정
caas generate "요구사항" --coverage 80
```
...
### Tip 2: 생성 시간 단축
```bash
# Haiku 모델 사용 (70% 빠름, 품질 약간 낮음)
caas generate "요구사항" --model haiku

# 캐싱 활성화 (재생성 시 빠름)
caas generate "요구사항" --cache
```
...
### 레퍼런스
- **[60_도메인_레퍼런스.md](../6_부록/60_도메인_레퍼런스.md)** - 17개 도메인 상세 설명

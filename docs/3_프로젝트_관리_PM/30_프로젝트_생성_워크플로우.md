...
```bash
# 초기 요구사항
caas refine "팀 할일 관리 시스템. 할일 추가, 우선순위 자동 추천, 공유 기능"

# 출력: 구체화된 요구사항
"팀 할일 관리 시스템.

기능:
- 할일 추가: 제목, 설명, 마감일, 담당자 입력
- 할일 조회: 팀원별/우선순위별 필터
- 우선순위 자동 분석: 마감일, 의존성, 작업량 기반
- 팀원 간 할일 공유: 읽기 전용 공유
- 마감일 알림: 이메일/Slack 알림
- 통계 리포트: 완료율, 지연율, 팀원별 현황
...
# 4. 검증
caas validate --validator golden-data --golden ./team-todo-project/golden_data.json
...
# 또는 전체 자동 실행
caas generate "구체화된 요구사항" --output ./team-todo-project --strict-quality
```
...
```bash
# 전체 검증
caas validate --validator all --project ./team-todo-project
```
...
caas qa report \
  --project ./team-todo-project \
  --format pdf \
  --output ./team-todo-final-qa-report.pdf
```
...
```bash
# 프로젝트 아카이브
caas download proj_123 ./team-todo-release-v1.0.zip

# 메타데이터 내보내기
caas export proj_123 --format json --output ./metadata.json
```

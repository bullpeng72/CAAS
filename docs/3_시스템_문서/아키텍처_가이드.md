...
- **BMAD 6-Phase 방법론**: 요구사항 분석부터 코드 생성, 품질 보증까지의 체계적인 워크플로우
- **Expert Agent Collaboration**: 5명의 전문가 에이전트 협업
- **도메인 기반 분류**: 8개 주요 도메인 자동 분류 및 최적 전략 선택
...
└─────────────────────────┬───────────────────────────────────┘
                          │
┌─────────────────────────┴───────────────────────────────────┐
│                  Application Layer (caas_app/)              │
│  (Shared Logic: Artifacts, Monitoring, Knowledge Graph)   │
└─────────────────────────┬───────────────────────────────────┘
                          │
┌─────────────────────────┴───────────────────────────────────┐
...
│
├── caas_cli/            # CLI 인터페이스 (20+ 파일)
├── caas_sdk/            # Python SDK
├── caas_app/            # 공유 애플리케이션 로직
│
├── data/                # 데이터 및 리소스
...
- **역할**: 6-Phase 워크플로우 전체를 조율하는 오케스트레이터.
- **주요 파일**: `engine.py`
...
- **Template Method Pattern**: `BMADEngine`의 `run` 메서드에서 전체 워크플로우의 뼈대를 정의하고, 각 단계의 구체적인 구현은 서브 클래스나 다른 모듈에 위임.

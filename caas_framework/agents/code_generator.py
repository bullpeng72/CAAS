...
"""
        ...
        guidelines = [
            ...
            "🚨 CRITICAL #4: tasks.py Task description에 {user_input} 플레이스홀더 사용 절대 금지!",
            "🚨 {user_input}은 generic 변수로 다른 도메인 특화 플레이스홀더({birthdate}, {keyword} 등)와 중복을 유발",
            "🚨 올바른 예시: description='생년월일 {birthdate}를 기반으로 별자리 계산' (도메인 변수만 사용)",
            "🚨 잘못된 예시: description=\"Using the input '{user_input}', 생년월일 '{birthdate}'\" (user_input 중복!)",

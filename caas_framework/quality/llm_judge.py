...
class EvaluationResult:
    ...
    def get_category_score(self, category: EvaluationCategory) -> float:
        """특정 카테고리의 평균 점수"""
        category_scores = [
            cs.score for cs in self.criteria_scores
...
class LLMJudge:
    ...
    def _build_evaluation_prompt(
        ...
        # Task 설명
        builder.add_task("""You are an expert code reviewer evaluating the quality of generated CrewAI code.
...
Be thorough but fair - code doesn't need to be perfect, but should meet professional standards.""")
    ...
    def _parse_evaluation_result(
        ...
        for score_data in evaluation_data.get("criteria_scores", []):
            ...
            matching_spec = next(
                (spec for spec in criteria_specs if spec["criterion"] == criterion_name),
                None
            )

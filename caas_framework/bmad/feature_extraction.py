...
class HierarchicalFeatureExtractor:
    ...
    async def extract_complete_features(
        ...
        for area in functional_areas:
            ...
            area_features = await self._extract_area_features(requirement, area, domain)
        ...
        missing_features = await self._verify_completeness(requirement, all_features)
    ...
    async def _identify_functional_areas(
        ...
        """
        prompt = (
            PromptBuilder("identify all functional areas from requirements")
            .add_task(
...
Be comprehensive - don't miss any area mentioned in the requirements."""
            ...
            .add_input(requirement=requirement, domain=domain or "General")
            .add_output_format(
                {"functional_areas": ["Area Name 1", "Area Name 2"]},
    ...
    async def _extract_area_features(
        self, requirement: str, area: str, domain: Optional[str] = None
    ) -> List[FeatureSpec]:
        """
        특정 영역의 모든 기능 추출
        ...
        """
        prompt = (
            PromptBuilder(f"extract all features for the '{area}' functional area")
            .add_task(
...
Be thorough - extract every feature related to {area}, even if small."""
    ...
    async def _verify_completeness(
        self, requirement: str, extracted_features: List[FeatureSpec]
    ) -> List[FeatureSpec]:
        """
        누락된 기능 검증 (교차 검증)
        ...
        features_summary = "\n".join(
            [f"- {f.name} ({f.id}): {f.description}" for f in extracted_features]
        )

        prompt = (
            PromptBuilder("verify completeness of feature extraction")
            .add_task(
...
Be critical and thorough. If everything is covered, return empty list."""
            ...
            .add_input(original_requirement=requirement)
            .add_context("Extracted Features", features_summary)
            .add_output_format(
                {
                    "missing_features": [
                        {
    ...
    def _prioritize_features(self, features: List[FeatureSpec]) -> List[FeatureSpec]:
        """
        우선순위 및 의존성 설정
        ...
        for priority, count in sorted(
            priority_dist.items(), key=lambda x: priority_order.get(x[0], 2)
        ):

...
async def _generate_frontend(agents_list, tasks_list, frontend_framework, verbose):
    """Generate frontend"""
    from caas_framework.codegen.frontend_generator import FrontendConfig, FrontendFramework, FrontendGenerator

    echo_progress(f"Generating {frontend_framework} frontend...")

    fw = FrontendFramework.STREAMLIT if frontend_framework == "streamlit" else FrontendFramework.REACT
    config = FrontendConfig(framework=fw, project_name="caas_project", port=8501)
    generator = FrontendGenerator()
    files = generator.generate(config=config)
...
async def _generate_docs(agents_list, tasks_list, golden_data, verbose):
    ...
    if golden_data is None:
        ...
        golden_data = ConcretizedRequirement(system_scope=SystemScope(project_name="Generated Project", purpose="Generated by CAAS"))
    elif isinstance(golden_data, dict):
        golden_data = ConcretizedRequirement(**golden_data)
    docs = generator.generate_all(
        golden_data=golden_data, agents=agents_list, tasks=tasks_list, config=config
    )

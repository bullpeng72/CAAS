...
class DocumentSharder(LoggerMixin):
    ...
    def shard_requirement(
        self,
        requirement: str,
        strategy: ShardingStrategy = ShardingStrategy.BY_SECTION
    ) -> ShardingResult:
        """
        요구사항을 조각으로 분할
        ...
        reduction = max(0.0, 1.0 - (total_with_overhead / total_tokens)) if total_tokens > 0 else 0.0
    ...
    def _shard_by_feature(self, text: str) -> List[DocumentShard]:
        ...
        for line in lines:
            # 기능 헤더 감지
            is_feature_header = any(re.search(pattern, line, re.IGNORECASE) for pattern in feature_patterns)
